"""
#Script 04 - Integration Between Environmental Degradation and Functional Profiles (GAM analysis)
...
# Node colours for network figure
NODE_COLS = {
    "HFP"           : "#E74C3C",    # Red
    "Limnological"  : "#2980B9",    # Blue
    "Biogeochemical": "#27AE60",    # Green
    "ARG"           : "#8E44AD",    # Purple
}
...
def load_all() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    gene_cpm : pd.DataFrame - gene x samples (CPM)
...
def fit_gam_single(x: np.ndarray, y: np.ndarray) -> dict:
    ...
    and return pseudo-R2, linear slope approximation (beta), and p-value.
    ...
    if PYGAM_AVAILABLE:
        try:
            ...
            # Pseudo-R2 = 1 - (sum_sq_resid / sum_sq_total)
...
def run_gam_layer(predictors:  list,
    ...
    """
    Run all single-predictor GAMs for one layer (predictor x response matrix).
    ...
    Returns
    -------
    df_layer : pd.DataFrame with all Predictor x Response combinations
    """
    rows = []
...
def run_all_layers(meta:     pd.DataFrame,
    ...
    # Gene CPM log10-transformed (responses treated as predictor in layer 3)
    gene_frame  = np.log10(gene_cpm + 1).T   # samples x genes
...
def plot_network(results: dict,
    ...
    Edge colour: blue (negative beta), red (positive beta).
    Edge width: scaled by |beta|.
    """
    if not NX_AVAILABLE:
    ...
    legend_patches += [
        plt.Line2D([0], [0], color="#E74C3C", lw=2, label="Positive beta"),
        plt.Line2D([0], [0], color="#2980B9", lw=2, label="Negative beta"),
    ]
...
if __name__ == "__main__":
    main()                                   (filtered, same as Script 03)
  - data/ARG_RPKM.csv          : ARG class RPKM matrix
